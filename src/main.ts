...
async function handleSearch(): Promise<void> {
  ...
  if (query === lastSearchQuery && currentState.status === 'success' && selectedRegions.length === 0){
    return;
  }
...
//Manejar los checkboxes del dropdown

const all = document.getElementById("all") as HTMLInputElement;
const regions = document.querySelectorAll(".region") as NodeListOf<HTMLInputElement>;
...
regions.forEach(region => {
  region.addEventListener("change", () => {
    if (region.checked) {
      ...
      selectedRegions = selectedRegions.filter(r => r !== region.value);
    }
    all.checked = selectedRegions.length === 0;
